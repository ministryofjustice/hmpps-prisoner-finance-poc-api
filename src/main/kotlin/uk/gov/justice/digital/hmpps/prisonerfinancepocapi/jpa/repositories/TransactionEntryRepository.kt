...
interface TransactionEntryRepository : JpaRepository<TransactionEntry, Long> {
  ...
  @Query("SELECT te FROM TransactionEntry te JOIN Transaction t ON te.transactionId = t.id WHERE t.date BETWEEN :startDate AND :endDate AND te.accountId IN :accountIds")
  fun findByDateBetweenAndAccountIdIn(startDate: Timestamp, endDate: Timestamp, accountIds: Collection<Long>): List<TransactionEntry>
}
